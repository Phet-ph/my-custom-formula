...
def my_custom_formula_updated(top_str, bottom_str):
    t1 = int(str(top_str).zfill(3)[0]) # ร้อยบน
    t2 = int(str(top_str).zfill(3)[1]) # สิบบน
    t3 = int(str(top_str).zfill(3)[2]) # หน่วยบน
    
    b1 = int(str(bottom_str).zfill(2)[0]) # สิบล่าง
    b2 = int(str(bottom_str).zfill(2)[1]) # หน่วยล่าง

    # 🎯 ชุดที่ 1: (ร้อยบน + หน่วยบน + หน่วยล่าง) สเต็ป +3, +6
    # ตรรกะ: ใช้เลขขอบกระดานมาเป็นตัวตั้ง
    start_num_1 = (t1 + t3 + b2) % 10
    set_1 = [start_num_1, (start_num_1 + 3) % 10, (start_num_1 + 6) % 10]

    # 🎯 ชุดที่ 2: (สิบบน + สิบล่าง) และ (ผลต่างหน่วยบน-หน่วยล่าง)
    # ตรรกะ: ใช้เลขแกนกลาง และ ความห่างของเลขท้าย
    sum_center = (t2 + b1) % 10
    diff_ends = abs(t3 - b2) % 10
    set_2 = [sum_center, diff_ends]
        ...
        try:
            ...
            if has_top_bot:
                ...
                if len(cleaned_df) < 2:
                    ...
                    st.markdown("### 🏆 สรุปภาพรวมความแม่นยำ สูตรชนหลักหน่วยและผลต่าง Unit Clash & Difference")
                    m1, m2 = st.columns(2)
