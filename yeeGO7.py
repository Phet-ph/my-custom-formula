...
# สูตรที่ 7: The Infinity Loop (เป้าหมาย 90-95%)
def my_custom_formula_updated(top_T, bot_T, top_P, bot_P):
    ...
    # 🎯 ชุดที่ 1: "The Flow" (ร้อยบนT + หน่วยบนT + หน่วยบนP)
    # ใช้การไหลของหลักหน่วยมาสร้างสเต็ป 3-6-9 (สเต็ปที่ครอบคลุมเลขกว้างที่สุด)
    flow_num = (t1_T + t3_T + t3_P) % 10
    set_1 = [flow_num, (flow_num + 3) % 10, (flow_num + 6) % 10, (flow_num + 9) % 10] # เพิ่มเป็น 4 ตัวเพื่อ Coverage

    # 🎯 ชุดที่ 2: "The Shadow" (สิบบนT + สิบล่างT) และเลขเงา
    # ตรรกะ: ดักจับจุดศูนย์กลางและเลขตรงข้าม (บวก 5)
    shadow_base = (t2_T + b1_T) % 10
    # ใช้ฐานเดิม, เลขข้างเคียง(+1), และเลขฝั่งตรงข้าม(+5)
    set_2 = [shadow_base, (shadow_base + 1) % 10, (shadow_base + 5) % 10]
