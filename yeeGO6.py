...
def my_custom_formula_updated(top_T, bot_T, top_P, bot_P):
    ...
    b2_P = int(str(bot_P).zfill(2)[1])

    # 🎯 ชุดที่ 1: "The Core Matrix" (ใช้ผลรวมสถานะปัจจุบันทั้งหมด)
    # ตรรกะ: ใช้พลังรวมของรอบปัจจุบันเป็นศูนย์กลาง
    core_sum = (t1_T + t2_T + t3_T + b1_T + b2_T) % 10
    set_1 = [core_sum, (core_sum + 3) % 10, (core_sum + 6) % 10]

    # 🎯 ชุดที่ 2: "The Momentum Guard" (ใช้ผลต่างข้ามรอบ + เลขพี่น้อง)
    # ตรรกะ: ดักจับเลขที่มักจะเคลื่อนที่ไปข้างหน้าหรือถอยหลัง 1 จังหวะจากความต่างรอบก่อน
    momentum_diff = abs(t3_T - b2_P) % 10
    # เพิ่ม "เลขกัน" อีก 1 ตำแหน่งในชุดที่ 2 เพื่อเพิ่ม Coverage
    set_2 = [momentum_diff, (momentum_diff + 1) % 10, (momentum_diff + 9) % 10]
        ...
        try:
            ...
            if has_top_bot:
                ...
                if len(cleaned_df) < 2:
                    ...
                    st.markdown("### 🏆 สรุปภาพรวมความแม่นยำสูตรที่4 The Power Synergy")
                    m1, m2 = st.columns(2)
