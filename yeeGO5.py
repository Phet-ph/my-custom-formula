...
# สูตรที่ 5: The Double-Lag Momentum
# ต้องรับค่าทั้งรอบปัจจุบัน (T) และรอบก่อนหน้า (P)
def my_custom_formula_updated(top_T, bot_T, top_P, bot_P):
    ...
    # รอบก่อนหน้า (P)
    t3_P = int(str(top_P).zfill(3)[2]) # หน่วยบนรอบก่อน
    b2_P = int(str(bot_P).zfill(2)[1]) # หน่วยล่างรอบก่อน

    # 🎯 ชุดที่ 1: "แรงเหวี่ยงหลักหน่วย" 
    # นำหน่วยบน(T) + หน่วยบน(P) + หน่วยล่าง(T)
    # ตรรกะ: เลขหน่วยมักจะมีการส่งต่อพลังงานกันระหว่างรอบ
    anchor = (t3_T + t3_P + b2_T) % 10
    set_1 = [anchor, (anchor + 3) % 10, (anchor + 6) % 10]

    # 🎯 ชุดที่ 2: "ค่าความต่างสะสม" (Lag Difference)
    # ตัวแรก: ผลรวมสิบบนปัจจุบัน + สิบล่างปัจจุบัน
    # ตัวสอง: ผลต่างของหน่วยล่างรอบปัจจุบัน และ รอบก่อนหน้า
    diff_lag = abs(b2_T - b2_P) % 10
    sum_current = (t2_T + b1_T) % 10
    
    set_2 = [sum_current, diff_lag]
        ...
        try:
            ...
            if has_top_bot:
                ...
                if len(cleaned_df) < 2:
                    ...
                    st.markdown("### 🏆 สรุปภาพรวมความแม่นยำ สูตรที่ 5 The Double-Lag Momentum โมเมนตัมหน่วงเวลา")
                    m1, m2 = st.columns(2)
